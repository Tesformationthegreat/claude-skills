...
## Relationship to the Product Manager skill

This skill replaces **Workflow #7 (Feature Prioritization)** that used to live inside the `product-manager` skill through v7. Starting with the product-manager skill v8, prioritization is handled here as a dedicated tool.

Why the split:

- Prioritization needed a richer framework than the RICE style scoring the old workflow used. VERDICT scores 7 weighted dimensions, runs a Kill Criteria gate, and flags compensatory tradeoffs. That is too much logic to bury inside a general purpose PM skill.
- Not every prioritization happens inside a PM workflow. Engineering backlogs, marketing campaigns, and founder to-do lists all benefit from VERDICT without needing the full PM stack.
- Bundling the Excel export keeps team scoring sessions possible, which the old inline-only workflow could not support.

**If you are upgrading from product-manager v7 to v8:** install both skills. The PM skill now points at VERDICT for any prioritization request, so the trigger still works. Nothing else changes.

**If you are new to the Tesformation skills:** you can install VERDICT standalone without the PM skill. It stands alone for any scoring or ranking use case.
